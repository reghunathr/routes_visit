...
# ====== Optional deps for contact picker & vCard ======
try:
...
def verify_password(raw: str, stored: str) -> bool:
    if _verify_external:
        try:
            ...
            pass
    # Fallback: plaintext compare (for dev only)
...
# ====== Contact helpers ======
def pick_contact_from_device():
    """
    Uses the Web Contacts Picker API (Android Chrome/Edge).
    Returns dict: {supported, cancelled, name, tel, email, error}
    """
    ...
    async () => {
      ...
      try {
        ...
        if (!contacts || !contacts.length) {
          return JSON.stringify({supported:true, cancelled:true});
        }
        const c = contacts[0] || {};
        ...
        return JSON.stringify({supported:true, error: String(e)});
      }
    }
    """
    raw = st_javascript(js)
    try:
        return json.loads(raw) if raw else {"supported": False}
    except Exception:
        return {"supported": False}

def parse_vcf_first_contact(file_bytes: bytes):
    """
    Parse a single-contact VCF and return dict with name, tel, email if present.
    """
    if vobject is None:
        return {"name": "", "tel": "", "email": ""}
    try:
        txt = file_bytes.decode("utf-8", errors="ignore")
        card = vobject.readOne(io.StringIO(txt))
        name = ""
        if hasattr(card, "fn"):
            name = str(card.fn.value)
        elif hasattr(card, "n"):
            name = " ".join([p for p in card.n.value if p])
        tel = ""
        if hasattr(card, "tel"):
            tel_field = card.tel
            if isinstance(tel_field, list) and tel_field:
                tel = str(tel_field[0].value)
            else:
                tel = str(tel_field.value)
        email = ""
        if hasattr(card, "email"):
            email_field = card.email
            if isinstance(email_field, list) and email_field:
                email = str(email_field[0].value)
            else:
                email = str(email_field.value)
        return {"name": name.strip(), "tel": tel.strip(), "email": email.strip()}
    except Exception:
        return {"name": "", "tel": "", "email": ""}
...
# ====== Mobile view (with contacts integration) ======
if vmode == "Mobile":
    ...
    # Contact picker + inputs + vCard fallback
    c_pick, c3, c4 = st.columns([0.9, 1, 1])
    with c_pick:
        if st.button("📇 Pick from Contacts", key=f"pick_btn_{rowkey}"):
            picked = pick_contact_from_device()
            if picked.get("supported") and not picked.get("cancelled"):
                if picked.get("name"):
                    st.session_state[f"contact_person_{rowkey}"] = picked["name"]
                if picked.get("tel"):
                    st.session_state[f"contact_number_{rowkey}"] = picked["tel"]
            elif not picked.get("supported"):
                st.info("Device contacts not available here. Try importing a .vcf below.")

    with c3:
        contact_person = st.text_input(
            "Contact Person",
            value=st.session_state.get(f"contact_person_{rowkey}", str(row_dict.get("Contact_Person","")).strip()),
            key=f"contact_person_{rowkey}"
        )
    with c4:
        contact_number = st.text_input(
            "Contact Number",
            value=st.session_state.get(f"contact_number_{rowkey}", str(row_dict.get("Contact_Number","")).strip()),
            key=f"contact_number_{rowkey}"
        )

    vcf = st.file_uploader("Or import from Contacts (.vcf)", type=["vcf"], key=f"vcf_{rowkey}", label_visibility="collapsed")
    if vcf is not None:
        parsed = parse_vcf_first_contact(vcf.read())
        if parsed.get("name"):
            st.session_state[f"contact_person_{rowkey}"] = parsed["name"]
        if parsed.get("tel"):
            st.session_state[f"contact_number_{rowkey}"] = parsed["tel"]
        st.rerun()
    ...
    if save_btn:
        contact_number_clean = "".join(ch for ch in contact_number if ch.isdigit() or ch == "+")
        updates = {
...
# ====== Desktop inline editor (with contacts support) ======
if vmode == "Desktop":
    choices = [(int(r), f"{int(r)} – {n}") for r, n in zip(df_view["_sheet_row"], df_view["Name"].astype(str))]
    ...
    # Contact picker + inputs + vCard fallback (desktop too)
    d_pick, d4, d5 = st.columns([0.9, 1, 1])
    with d_pick:
        if st.button("📇 Pick from Contacts", key=f"pick_btn_{rowkey}"):
            picked = pick_contact_from_device()
            if picked.get("supported") and not picked.get("cancelled"):
